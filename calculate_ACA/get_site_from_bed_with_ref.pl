#! perl -w
# get the location of ACA motif on the fragment 
# perl get_site_from_bed_with_ref.pl -f ref_2l.fa -m ref_motif_exon -b file.bed > file.digest
	...
	for $i(0..$#loc1s){
        ...
        foreach $num(@nums){ ## deal with each ACA motif, convert the location of the fragment to the real location on the ref genome
        	...
        	if($num < $lens[$i]){
                ...
                push @plus,$site_plus;
                if($num+1>5){
                        $inner_plus{$site_plus}++; ## count the internal ACA motif
                }elsif($num==0){
                        $outer_plus{$site_plus}++; ## count the ACA motif on the terminal
                }
                }
                last;
        	}
